...
describe('Sites build staging', () => {
  ...
  it('places the Vite app in the client asset directory and emits the worker', async () => {
    ...
    const clientHtml = await readFile(resolve(fixture, 'dist', 'client', 'index.html'), 'utf8')
    expect(clientHtml).toContain('<!doctype html>')
    expect(clientHtml).toContain('<script src="/runtime-config.js"></script>')
    expect(await readFile(resolve(fixture, 'dist', 'client', 'assets', 'app.js'), 'utf8')).toBe('export {}')
    const worker = await readFile(resolve(fixture, 'dist', 'server', 'index.js'), 'utf8')
    expect(worker).toContain("url.pathname === '/runtime-config.js'")

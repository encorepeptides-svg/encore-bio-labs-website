...
create index client_protocols_user_id_idx on public.client_protocols (user_id, status);
...
create policy "support client update own" on public.support_threads for update to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "audit authenticated append" on public.audit_logs for insert to authenticated with check (actor_id = auth.uid());

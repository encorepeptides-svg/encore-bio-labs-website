...
function serverRow(overrides: Partial<PortalOnboardingRow> = {}): PortalOnboardingRow {
  return {
    date_of_birth: null, goals: ['wellness'], research_interests: ['longevity-cellular-health'],
    interested_products: ['nad-plus'], activity_level: 'Moderate', exercise_frequency: '3', average_sleep_hours: 7,
    water_consistency: 4, energy_rating: 4, stress_rating: 2, wellness_rating: 4,
...
describe('portal onboarding draft persistence', () => {
  it('restores newer server draft arrays, ratings and consent choices over canonical values', () => {
    const restored = restorePortalOnboardingForm(baseForm(), serverRow({
      draft_data: {
        goals: ['recovery', 'energy'], interestedProducts: ['retatrutide', 'nad-plus'],
        water: '5', terms: true, privacy: true, photos: true,
      },
  ...
  it('does not restore a draft after it is marked completed', () => {
    const restored = restorePortalOnboardingForm(baseForm(), serverRow({
      draft_data: { goals: ['recovery'] },
      draft_completed_at: '2026-07-23T13:00:00Z',
    }))
    expect(restored.goals).toEqual(['wellness'])
  })

  it('carries no body-composition or appetite field through the draft round trip', () => {
    const restored = restorePortalOnboardingForm(baseForm(), serverRow())
    for (const key of ['height', 'startingWeight', 'currentWeight', 'waist', 'appetite', 'units']) {
  ...
  it.each(['en', 'es'] as const)('provides clear individual rating descriptions in %s', (locale) => {
    const expected = locale === 'en'
      ? ['1 = rarely, 5 = consistently', '1 = very low, 5 = very high', '1 = very poor, 5 = excellent']
      : ['1 = rara vez, 5 = de forma constante', '1 = muy bajo, 5 = muy alto', '1 = muy baja, 5 = muy alta', '1 = muy malo, 5 = excelente']
    const copy = [

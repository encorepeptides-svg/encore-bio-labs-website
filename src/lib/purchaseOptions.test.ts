...
describe('purchase option pricing', () => {
  it('validates every active Retatrutide strength, label, SKU, and price path', () => {
    const expected = [
      { label: '10 mg', strength: 10, base: 130, badge: 'Starter' },
      { label: '15 mg', strength: 15, base: 155, badge: undefined },
      { label: '20 mg', strength: 20, base: 180, badge: 'Most Popular' },
      { label: '25 mg', strength: 25, base: 205, badge: undefined },
      { label: '30 mg', strength: 30, base: 230, badge: 'Best Value' },
    ]
    ...
    for (const row of expected) {
      ...
      expect(kit.pricePerMeasure).toBe((row.base + 10) / row.strength)
      expect(three.linePrice).toBeCloseTo(row.base * 3 * 0.92)
      expect(five.linePrice).toBeCloseTo(row.base * 5 * 0.88)
  ...
  it('defaults kit-eligible products to one Complete Kit', () => {
    ...
    expect(quote.pricePerMeasure).toBe(14)
  })
  ...
  it('applies 8% to three vials and charges only one optional kit', () => {
    ...
    expect(quote.unitPrice).toBeCloseTo(119.6)
    expect(quote.savings).toBeCloseTo(31.2)
    expect(quote.linePrice).toBeCloseTo(368.8)
    expect(quote.kitIncluded).toBe(true)
  })
  ...
  it('applies 12% to five vials without a kit when declined', () => {
    ...
    expect(quote.linePrice).toBeCloseTo(572)
    expect(quote.savings).toBeCloseTo(78)
  ...
  it('uses total measured amount for blends and IU-based packages', () => {
    ...
    expect(hcgKit.pricePerMeasure).toBe(0.014)
    expect(unitMoney(hcgKit.pricePerMeasure!)).toBe('$0.014')
  })
  ...
  it('recognizes a temporarily unavailable product without deleting its price', () => {
    ...
    expect(unavailable.variants[0].price).toBe(130)
  })
  ...
  it('normalizes purchase-option switching and invalid combinations', () => {
    ...
    expect(pack).toEqual({ optionId: 'multipack', packSize: 3, includeKit: true })
    expect(changePurchaseOption(retatrutide, kit, 'multipack'))
      .toEqual({ optionId: 'multipack', packSize: 3, includeKit: false })

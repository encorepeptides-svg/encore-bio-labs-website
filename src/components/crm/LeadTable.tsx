...
export function LeadTable({ leads, onSelect }: { leads: Lead[]; onSelect: (lead: Lead) => void }) {
  ...
  const formatDate = (value: string) => new Intl.DateTimeFormat(locale === 'es' ? 'es-MX' : 'en-US', { month: 'short', day: 'numeric' }).format(new Date(value))
  ...
  return (
    <section className="rounded-[1.75rem] border border-slate-900/10 bg-white/78 p-4 shadow-[0_24px_80px_rgba(7,23,36,0.08)] backdrop-blur-2xl sm:p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-700">{t('leadTable')}</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-[-0.04em] text-[#071724]">{t('pipeline')}</h2>
        </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        <Filter label={t('filterStatus')} value={status} onChange={(value) => setStatus(value as LeadStatus | 'all')} options={statuses.map(String)} labels={{ all: t('all'), ...statusLabels }} />
        <Filter label={t('filterProduct')} value={product} onChange={setProduct} options={['all', ...productOptions]} labels={{ all: t('all') }} />
        <Filter label={t('filterCity')} value={city} onChange={setCity} options={['all', ...cityOptions]} labels={{ all: t('all') }} />
        <Filter label={t('filterSource')} value={source} onChange={setSource} options={['all', ...sourceOptions]} labels={{ all: t('all') }} />
        <Filter label={t('filterSort')} value={scoreSort} onChange={(value) => setScoreSort(value as 'desc' | 'asc')} options={['desc', 'asc']} labels={{ desc: t('sortHigh'), asc: t('sortLow') }} />
      </div>
        ...
        <table className="w-full min-w-[68rem] border-separate border-spacing-0 text-left text-sm">
          <thead>
            <tr className="text-xs uppercase tracking-[0.14em] text-slate-400">
              {[t('columnLead'), t('columnStatus'), t('columnScore'), t('columnProducts'), t('columnCity'), t('columnSource'), t('columnCreated')].map((heading) => (
              ...
              <tr key={lead.id} className="align-top">
                <td className="border-b border-slate-900/10 px-3 py-4">
                  <button type="button" onClick={() => onSelect(lead)} className="text-left">
                    ...
                    <span className="mt-1 block text-xs text-slate-500">{lead.email || lead.phone || t('noContact')}</span>
                  </button>
                </td>
      ...
      >
        {options.map((option) => (
          <option key={option} value={option}>
            {labels[option] ?? (statuses.includes(option as LeadStatus) ? leadStatusLabel(option as LeadStatus) : option)}
          </option>
        ))}
      </select>
    </label>

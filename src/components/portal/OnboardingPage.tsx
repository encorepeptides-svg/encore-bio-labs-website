...
function initialForm(identity: ReturnType<typeof usePortalAuth>['identity'], locale: 'en' | 'es'): PortalOnboardingForm {
  return {
    ...
    water: '3', energy: '3', stress: '3', wellness: '3',
...
export function OnboardingPage() {
  ...
  const agreementItems = useMemo<Array<ConsentChecklistItem<AgreementKey>>>(() => [
    ...
    { key: 'noMedical', title: t('noMedicalAck'), summary: t('noMedicalAckSummary'), details: t('noMedicalAckDetails'), required: true },
    { key: 'electronic', title: t('electronicConsentAck'), summary: t('ackElectronicSummary'), details: t('ackElectronicDetails'), required: true },
    { key: 'progressData', title: t('progressDataConsent'), summary: t('progressDataConsentSummary'), details: t('progressDataConsentDetails'), required: true },
    { key: 'photos', title: t('progressPhotoConsent'), summary: t('progressPhotoConsentSummary'), details: t('progressPhotoConsentDetails'), required: false },
  ], [t])
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      // No height/weight/waist/appetite: the portal does not record body
      // composition or appetite. Those columns keep their historical values.
      const { error: onboardingError } = await supabase.from('onboarding_profiles').upsert({
        user_id: identity.user.id, date_of_birth: form.dateOfBirth || null, goals: form.goals,
        ...
        water_consistency: Number(form.water), energy_rating: Number(form.energy),
    ...
    <PortalShell>
      <form onSubmit={submit} aria-busy={loading}>
        ...
        <div className="mt-9 min-h-[24rem] rounded-[1.5rem] bg-[#f8faf9] p-5 sm:p-8">
          ...
          {step === 1 ? <div className="grid gap-5 sm:grid-cols-2"><Input label={t('dateOfBirthLabel')} type="date" value={form.dateOfBirth} onChange={(value) => update('dateOfBirth', value)} /></div> : null}
          {step === 2 ? <ChoiceButtons items={goals} selected={form.goals} onToggle={(value) => toggleArray('goals', value)} /> : null}
          {step === 3 ? <div><p className="max-w-2xl text-sm leading-6 text-slate-600">{t('researchInterestsStepCopy')}</p><h2 className="mt-6 text-lg font-semibold">{t('researchInterestCategories')}</h2><div className="mt-3"><ChoiceButtons items={researchInterests} selected={form.researchInterests} onToggle={(value) => toggleArray('researchInterests', value)} /></div><h2 className="mt-7 text-lg font-semibold">{t('researchInterestProducts')}</h2><p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">{t('researchInterestProductsHelp')}</p><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{productChoices.map((product) => { const selected = form.interestedProducts.includes(product.slug); return <button type="button" key={product.slug} aria-pressed={selected} onClick={() => toggleArray('interestedProducts', product.slug)} className={`min-h-28 rounded-xl border p-4 text-left text-sm outline-none focus-visible:ring-4 focus-visible:ring-teal-100 ${selected ? 'border-teal-700 bg-teal-50 text-[#071724]' : 'border-slate-200 bg-white text-slate-700 hover:border-teal-500'}`}><span className="block font-semibold">{product.name}</span><span className={`mt-2 block text-xs font-normal leading-5 ${selected ? 'text-teal-950/75' : 'text-slate-500'}`}>{product.catalogTagline}</span></button> })}</div></div> : null}
          {step === 4 ? <div className="grid gap-5 sm:grid-cols-2"><Select label={t('activityLevelLabel')} value={form.activity} onChange={(value) => update('activity', value)} options={[{ value: 'Low', label: t('activityLow') }, { value: 'Moderate', label: t('activityModerate') }, { value: 'High', label: t('activityHigh') }]} placeholder={t('selectPlaceholder')} /><Input label={t('exerciseSessionsLabel')} type="number" min="0" max="14" step="1" value={form.exercise} onChange={(value) => update('exercise', value)} /><Input label={t('averageSleepLabel')} type="number" min="0.1" max="24" step="0.1" value={form.sleep} onChange={(value) => update('sleep', value)} /><RatingSelect label={t('waterRatingLabel')} helper={t('waterRatingHelp')} value={form.water} onChange={(value) => update('water', value)} /><RatingSelect label={t('energyRatingLabel')} helper={t('energyRatingHelp')} value={form.energy} onChange={(value) => update('energy', value)} /><RatingSelect label={t('stressRatingLabel')} helper={t('stressRatingHelp')} value={form.stress} onChange={(value) => update('stress', value)} /><RatingSelect label={t('wellnessRatingLabel')} helper={t('wellnessRatingHelp')} value={form.wellness} onChange={(value) => update('wellness', value)} /></div> : null}

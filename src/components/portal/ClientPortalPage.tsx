...
export function ClientPortalPage({ section = 'overview' }: { section?: string }) {
  ...
  return <PortalShell><p className="text-xs font-bold uppercase tracking-[.18em] text-teal-700">{t('accountOverview')}</p><h1 className="mt-3 text-4xl font-semibold tracking-[-.055em] sm:text-5xl">{t('welcomeMessage',{name:identity?.profile.preferred_name||identity?.profile.legal_name||t('defaultClientName')})}</h1><p className="mt-4 max-w-2xl leading-7 text-slate-600">{t('overviewBody')}</p>
    ...
    <div className="mt-8 grid gap-4 md:grid-cols-3"><Status title={t('statusAccountStatus')} value={accountStatus}/><Status title={t('statusNewDocuments')} value={t('statusNoDocuments')}/><Status title={t('statusNotifications')} value={t('statusNoNotifications')} icon/></div>
  </PortalShell>
}
...
function SectionEmpty({section}:{section:string}){
  const { t } = useTranslation('portal')
  const title=({ progress:t('navProgress'), 'check-ins':t('navCheckIns'), calculators:t('navCalculators'), orders:t('navOrders'), documents:t('navDocuments'), support:t('navSupport'), notifications:t('navNotifications'), profile:t('navProfile'), security:t('navSecurity') } as Record<string,string>)[section] ?? t('clientPortalLabel')

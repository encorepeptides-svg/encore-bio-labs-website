...
export function AdminPortalPage({ section = 'overview' }: { section?: string }) {
  ...
  const sectionTitle = ({ content: t('adminNavContent'), clients: t('adminNavClients'), orders: t('adminNavOrders'), documents: t('adminNavDocuments'), support: t('adminNavSupport'), 'audit-log': t('adminNavAudit'), settings: t('adminNavSettings') } as Record<string,string>)[section] ?? section

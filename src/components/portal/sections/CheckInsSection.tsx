...
export function CheckInsSection() {
  ...
  const [form, setForm] = useState({ energy: '3', sleep: '3', stress: '3', concern: false, notes: '' })
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await saveCheckin(identity.user.id, {
        ...
        scores: { energy: Number(form.energy), sleep: Number(form.sleep), stress: Number(form.stress) },
  ...
  return <>
    ...
    <Card className="mt-8">
      ...
      <form onSubmit={submit} className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {([['energy', t('energyRatingLabel')], ['sleep', t('checkinSleepRating')], ['stress', t('stressRatingLabel')]] as const).map(([key, label]) => <FieldLabel key={key} label={label}><select value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} className="portal-input">{['1', '2', '3', '4', '5'].map((value) => <option key={value}>{value}</option>)}</select></FieldLabel>)}
    ...
    <LoadState loading={loading} error={error} onRetry={reload}>
      {data?.length ? <div className="mt-6 grid gap-3">{data.map((checkin) => <div key={checkin.id} className="flex flex-wrap items-center justify-between gap-3 rounded-[1.25rem] bg-white px-5 py-4 text-sm shadow-[0_10px_35px_rgba(7,23,36,.05)]"><p className="font-semibold">{t('checkinsWeekOf', { date: formatDate(checkin.week_start) })}</p><div className="flex flex-wrap gap-2 text-slate-600"><span>{t('energyRatingLabel')}: {checkin.scores.energy ?? '—'}/5</span><span>{t('checkinSleepRating')}: {checkin.scores.sleep ?? '—'}/5</span><span>{t('stressRatingLabel')}: {checkin.scores.stress ?? '—'}/5</span></div>{checkin.support_concern ? <Badge tone="attention">{t('checkinConcernBadge')}</Badge> : null}</div>)}</div> : <EmptyCard title={t('checkinsEmptyTitle')} copy={t('checkinsEmptyCopy')} />}
    </LoadState>
  </>
}

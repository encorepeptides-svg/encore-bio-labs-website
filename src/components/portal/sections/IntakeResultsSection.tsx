    ...
    <LoadState loading={loading} error={error} onRetry={reload}>
      {data ? <>
        ...
        <Card className="mt-4">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-5">{([[t('waterRatingLabel'), data.water_consistency], [t('energyRatingLabel'), data.energy_rating], [t('stressRatingLabel'), data.stress_rating], [t('wellnessRatingLabel'), data.wellness_rating]] as const).map(([label, value]) => <div key={label} className="rounded-[1.1rem] bg-white p-4 text-center"><p className="text-2xl font-semibold">{value ?? '—'}<span className="text-sm text-slate-400">/5</span></p><p className="mt-1 text-xs font-semibold text-slate-500">{label}</p></div>)}</div>
        </Card>

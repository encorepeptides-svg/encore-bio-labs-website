...
export function ProgressSection() {
  ...
  const [form, setForm] = useState({ date: new Date().toISOString().slice(0, 10), energy: '3', wellness: '3', notes: '' })
      ...
      {entries.length ? <>
        <div className="mt-4 overflow-x-auto rounded-[1.5rem] border border-slate-900/8 bg-white">
          <table className="w-full min-w-[34rem] text-left text-sm">
            <thead><tr className="bg-slate-50 text-xs font-bold uppercase tracking-wide text-slate-500"><th className="px-5 py-3">{t('progressDate')}</th><th className="px-5 py-3">{t('energyRatingLabel')}</th><th className="px-5 py-3">{t('wellnessRatingLabel')}</th><th className="px-5 py-3">{t('progressNotes')}</th></tr></thead>
            <tbody>{entries.map((entry) => <tr key={entry.id} className="border-t border-slate-900/6"><td className="px-5 py-3 font-semibold">{formatDate(entry.entry_date)}</td><td className="px-5 py-3">{entry.scores.energy ?? '—'}</td><td className="px-5 py-3">{entry.scores.wellness ?? '—'}</td><td className="px-5 py-3 text-slate-500">{entry.notes ?? ''}</td></tr>)}</tbody>
          </table>
        </div>

  ...
  return <>
    ...
    <Card className="mt-8 border-teal-900/10 bg-[linear-gradient(145deg,#edf9f6,#ffffff)]">
      ...
      <form onSubmit={submit} className="mt-6 grid gap-5 sm:grid-cols-2">
        ...
        <FieldLabel label={t('feedbackCategory')}><select className="portal-input" value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })}><option value="service">{t('feedbackCategoryService')}</option><option value="lab paperwork">{t('feedbackCategoryDocumentation')}</option><option value="fulfillment">{t('feedbackCategoryFulfillment')}</option><option value="support">{t('feedbackCategorySupport')}</option></select></FieldLabel>

...
export function ProtectedPortal({ children, admin = false, allowOnboarding = false }: { children: ReactNode; admin?: boolean; allowOnboarding?: boolean }) {
  ...
  if (decision === 'unauthenticated') {
    const currentPath = typeof window === 'undefined' ? (admin ? '/admin' : '/portal') : window.location.pathname.replace(/^\/es(?=\/)/, '')
    const signInHref = admin
      ? `${path('/client-login')}?next=${encodeURIComponent(currentPath)}`
      : path('/client-login')
    return <PortalGate
      title={admin ? t('gateAdminSignInTitle') : t('gateSignInTitle')}
      copy={admin ? t('gateAdminSignInCopy') : t('gateSignInCopy')}
      action={admin ? t('gateAdminSignInAction') : t('gateSignInAction')}
      href={signInHref}
      admin={admin}
    />
  }
...
function PortalGate({ title, copy, action, href, admin = false }: { title:string; copy:string; action?:string; href?:string; admin?: boolean }) {
  const { t } = useTranslation('portal')
  return <main id="main-content" className="grid min-h-[70vh] place-items-center bg-[#f5f6f3] px-5"><div className="max-w-xl rounded-[2rem] bg-white p-8 text-center shadow-[0_28px_90px_rgba(7,23,36,.1)]"><p className="text-xs font-bold uppercase tracking-[.18em] text-teal-700">{admin ? t('gateAdminTitleLabel') : t('gateTitle')}</p><h1 className="mt-4 text-4xl font-semibold tracking-[-.055em]">{title}</h1><p className="mt-4 leading-7 text-slate-600">{copy}</p>{action&&href?<a href={href} className="mt-7 inline-flex min-h-12 items-center rounded-full bg-[#071724] px-6 text-sm font-semibold text-white">{action}</a>:null}</div></main>
}

          ...
          <tbody>{rows.map((order) => {
            ...
            return <tr key={order.id} className="border-t border-slate-900/6 align-top">
              <td className="px-5 py-3">
                ...
                {order.local_fulfillment_method ? <p className="mt-1 text-xs text-slate-500">{order.local_fulfillment_method.replaceAll('_', ' ')}{order.address_verification.coverageCenterPostalCode ? ` · center ${order.address_verification.coverageCenterPostalCode}` : ''}{order.delivery_distance_miles !== null ? ` · ${order.delivery_distance_miles.toFixed(1)} mi` : ''}</p> : null}
              </td>

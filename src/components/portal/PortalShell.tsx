import { Bell, Calculator, Camera, ClipboardCheck, ClipboardList, FileText, FlaskConical, Gauge, Headphones, LogOut, Mail, MessageSquareQuote, Package, ShieldCheck, Sparkles, TrendingUp, UserRound } from 'lucide-react'
...
export function PortalShell({ children, admin = false }: { children: ReactNode; admin?: boolean }) {
  ...
  const clientNav = [
    ...
    ['/portal/research-matches', t('navResearchMatches'), Sparkles], ['/portal/calculators', t('navCalculators'), Calculator], ['/portal/research-media', t('navResearchMedia'), Camera], ['/portal/feedback', t('navFeedback'), MessageSquareQuote], ['/portal/documents', t('navDocuments'), FileText],
  ...
  return <main id="main-content" className="min-h-screen bg-[#f5f6f3] px-4 py-5 sm:px-6 lg:p-8"><div className="mx-auto grid max-w-[92rem] gap-5 lg:grid-cols-[17rem_1fr]">
    <aside className="rounded-[1.75rem] bg-[#071724] p-5 text-white shadow-[0_25px_80px_rgba(7,23,36,.16)] lg:sticky lg:top-5 lg:h-[calc(100vh-2.5rem)] lg:overflow-y-auto"><BrandLogoLink className="rounded-xl bg-white/95 px-3 py-2 shadow-sm" imageClassName="h-8 sm:h-8" /><p className="mt-7 text-[.65rem] font-bold uppercase tracking-[.18em] text-teal-200">{admin?t('adminLabel'):t('clientPortalLabel')}</p><nav className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-1">{nav.map((entry)=>{const href=entry[0];const label=entry[1];const Icon=entry.length>2?entry[2]:null;const active=portalPath===href;return <a key={href} href={path(href)} className={`flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition ${active?'bg-white text-[#071724]':'text-slate-300 hover:bg-white/8 hover:text-white'}`}>{Icon?<Icon size={16}/>:null}{label}</a>})}</nav><button onClick={()=>void logout().then(()=>window.location.assign(path('/client-login')))} className="mt-5 flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-sm font-semibold text-slate-300 hover:bg-white/8"><LogOut size={16}/>{t('secureLogout')}</button></aside>

...
export function RetatrutideResearchNetwork() {
  ...
  return (
    <div className="mt-10 grid gap-10 xl:grid-cols-[0.28fr_0.72fr] xl:items-end">
      <div className="order-2 pb-8 xl:order-1 xl:pb-12">
        <p className="max-w-xs text-3xl font-semibold leading-tight tracking-[-0.045em] text-slate-900">Multiple dimensions. One investigational molecule.</p>
        <p className="mt-4 max-w-xs text-sm leading-6 text-slate-600">Reported outcomes vary by trial population, duration and investigational dose.</p>
      </div>

      <div className="order-1 xl:order-2">
        <div data-research-network="orbital" className="relative hidden min-h-[46rem] overflow-hidden rounded-[2rem] border border-white/70 bg-[radial-gradient(circle_at_50%_48%,rgba(20,184,166,.14),transparent_23%),linear-gradient(145deg,rgba(255,255,255,.82),rgba(228,248,244,.58)_55%,rgba(229,246,250,.7))] shadow-[inset_0_1px_0_rgba(255,255,255,.9)] min-[900px]:block">
          ...
          <motion.div initial={reducedMotion?false:{opacity:0,scale:.94}} whileInView={{opacity:1,scale:1}} viewport={{once:true}} transition={{duration:.65}} className="absolute left-1/2 top-1/2 flex size-56 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full border border-emerald-300/25 bg-[radial-gradient(circle_at_35%_25%,#18374a,#071724_62%)] p-7 text-center text-white shadow-[0_0_0_15px_rgba(20,184,166,.06),0_30px_90px_rgba(7,23,36,.28)]">
            ...
            <p className="relative text-xs font-bold uppercase tracking-[0.18em] text-teal-200">Triple-Pathway</p><p className="relative mt-2 text-2xl font-semibold leading-tight tracking-[-0.04em]">Metabolic Research</p><p className="relative mt-3 text-[0.65rem] font-semibold tracking-[0.08em] text-slate-300">GLP-1 + GIP + Glucagon</p>
          </motion.div>
        ...
        <div data-research-network="vertical" className="relative min-[900px]:hidden">
          <div className="relative mx-auto flex size-48 flex-col items-center justify-center rounded-full bg-[#071724] p-6 text-center text-white shadow-[0_0_0_12px_rgba(20,184,166,.07),0_26px_70px_rgba(7,23,36,.22)]"><p className="text-[0.65rem] font-bold uppercase tracking-[0.16em] text-teal-200">Triple-Pathway</p><p className="mt-2 text-xl font-semibold">Metabolic Research</p><p className="mt-2 text-[0.6rem] text-slate-300">GLP-1 + GIP + Glucagon</p></div>

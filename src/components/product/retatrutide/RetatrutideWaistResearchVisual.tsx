  ...
  return (
    ...
    >
      ...
      <figure className="mt-4 flex min-h-0 flex-1 items-center justify-center sm:mt-5">
        <img src={locale === 'es' ? waistResearchJourneyEs : waistResearchJourney} alt={t('waistVisualImageAlt', { metric: metricWithPrefix })} width="1672" height="941" loading="lazy" decoding="async" className="block h-auto w-full object-contain lg:h-full" />
      </figure>
      <p className="border-t border-teal-900/8 px-6 py-4 text-left text-[.68rem] leading-5 text-slate-500 sm:px-8 lg:px-9">{t('waistVisualFootnote', { duration })}</p>
    </motion.article>
  )
}

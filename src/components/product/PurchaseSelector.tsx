...
export function PurchaseSelector({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return (
    <section className={cn('rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-[0_24px_70px_rgba(7,23,36,0.09)] sm:p-6', compact && 'p-4 sm:p-5')} aria-labelledby={`purchase-options-${product.slug}`}>
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-700">Configure your order</p>
      <h2 id={`purchase-options-${product.slug}`} className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[#071724]">Purchase Options</h2>

      {product.variants.length > 1 ? (
        <fieldset className="mt-5">
          <legend className="text-sm font-semibold text-[#071724]">Strength or format</legend>
          <div className="mt-2 flex flex-wrap gap-2">
            {product.variants.map((entry) => {
              const badge = entry.price > 0 ? getRetatrutideVariantBadge(product, entry) : undefined
              return (
                <button key={`${entry.label}-${entry.format}`} type="button" aria-pressed={entry === variant} onClick={() => { setVariant(entry); track('strength_selected', { productId: product.slug, sku: entry.sku, strength: entry.label }) }} className={cn('min-h-11 rounded-xl border px-3 py-2 text-sm font-semibold outline-none focus:ring-4 focus:ring-teal-100', entry === variant ? 'border-teal-700 bg-teal-50 text-teal-900' : 'border-slate-200 bg-white text-slate-600')}>
                  {entry.label} · {entry.price > 0 ? money(entry.price) : 'Request availability'}{badge ? <span className="ml-2 text-[10px] uppercase tracking-wide text-teal-700">{badge}</span> : null}
                </button>
              )
            })}
          </div>
        ...
        <p className="mt-4 text-sm font-semibold text-[#071724]">{variant.label} · {variant.format}</p>
      )}

      {!isProductPurchasable(product) || variant.price <= 0 ? (
        <div className="mt-5 rounded-2xl border border-slate-200 bg-[#f8fafc] p-5">
          <p className="text-sm font-semibold text-[#071724]">This selection is temporarily unavailable.</p>
          <p className="mt-2 text-xs leading-5 text-slate-500">Encore will confirm current availability and fulfillment timing through the existing inquiry process.</p>
          <a href={`/intake?product=${encodeURIComponent(product.slug)}`} className="mt-4 inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[#071724] px-5 text-sm font-semibold text-white">Request availability</a>
        </div>
      ) : (
      <>

      <fieldset className="mt-5">
        <legend className="sr-only">Choose one purchase option</legend>
        <div className="grid gap-3">
          <label className={optionClass(selection.optionId === 'vial-only')}>
            ...
            <span className="min-w-0"><span className="flex flex-wrap items-baseline justify-between gap-2"><strong className="text-sm text-[#071724]">{product.purchaseRules.productType === 'research-vial' ? 'Vial Only' : 'Product Only'}</strong><strong className="text-base text-[#071724]">{money(variant.price)}</strong></span><span className="mt-1 block text-xs leading-5 text-slate-500">{product.purchaseRules.productType === 'research-vial' ? 'Product vial in standard Encore packaging' : 'Individual product in standard Encore packaging'}</span></span>
          </label>
          ...
          {product.purchaseRules.kitEligible ? (
            <label className={optionClass(selection.optionId === 'complete-kit')}>
              ...
              <span className="min-w-0"><span className="flex flex-wrap items-baseline justify-between gap-2"><span className="inline-flex flex-wrap items-baseline gap-2"><strong className="text-sm text-[#071724]">Encore Complete Kit</strong><span className="whitespace-nowrap rounded-full bg-teal-700 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-white">Most Popular</span></span><strong className="text-base text-[#071724]">{money(variant.price + getKitPremium(product))}</strong></span><span className="mt-1 block text-xs leading-5 text-slate-500">Product vial plus correctly matched research accessories (+{money(getKitPremium(product))})</span></span>
            </label>
          ) : null}
          ...
          {product.purchaseRules.multipackEligible && smallestMultipackQuote ? (
            <label className={optionClass(selection.optionId === 'multipack')}>
              ...
              <span className="min-w-0"><span className="flex flex-wrap items-baseline justify-between gap-2"><strong className="text-sm text-[#071724]">Multi-Vial Research Pack</strong><strong className="text-base text-[#071724]">from {money(smallestMultipackQuote.linePrice)}</strong></span><span className="mt-1 block text-xs leading-5 text-slate-500">Reduced per-vial pricing for repeat research purchases</span></span>
            </label>
          ) : null}
        </div>
      </fieldset>
      ...
      {selection.optionId === 'multipack' ? (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-[#f8fafc] p-4">
          <label className="text-sm font-semibold text-[#071724]">Pack quantity<select value={selection.packSize} onChange={(event) => setSelection((current) => ({ ...current, packSize: Number(event.target.value) }))} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 outline-none focus:ring-4 focus:ring-teal-100">{product.purchaseRules.multipackQuantities.map((size) => <option key={size} value={size}>{size} vials</option>)}</select></label>
          {product.purchaseRules.kitEligible ? <label className="mt-3 flex min-h-11 items-center gap-3 text-sm font-semibold text-slate-700"><input type="checkbox" checked={selection.includeKit} onChange={(event) => setSelection((current) => ({ ...current, includeKit: event.target.checked }))} className="size-4 accent-teal-700" />Add one Complete Kit (+{money(getKitPremium(product))})</label> : null}
        </div>
      ) : null}

      {quote.kitIncluded ? <div className="mt-4 rounded-2xl bg-teal-50 p-4"><p className="flex items-center gap-2 text-sm font-semibold text-teal-950"><PackageCheck size={16} />One Complete Kit included</p><ul className="mt-2 grid gap-1 text-xs leading-5 text-teal-900 sm:grid-cols-2"><li>• Product vial</li><li>• Matched BAC water, where applicable</li><li>• Research accessories and prep pads</li><li>• Handling and storage information</li><li>• Encore premium packaging</li></ul></div> : null}

      <div className="mt-5 rounded-2xl border border-slate-200 p-4" aria-live="polite" aria-atomic="true">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Order total</p><p className="mt-1 text-3xl font-semibold tracking-[-0.04em] text-[#071724]">{money(quote.linePrice)}</p></div><div className="text-right text-xs leading-5 text-slate-500">{quote.packSize > 1 ? <><p>{money(quote.unitPrice)} effective per vial</p><p className="font-semibold text-emerald-700">Save {money(quote.savings)} ({quote.savingsPercent}%)</p></> : null}{quote.pricePerMeasure && variant.unitType ? <p>{unitMoney(quote.pricePerMeasure)} per {variant.unitType}</p> : null}</div></div>
      </div>

      {variant.price > 0 ? <button type="button" onClick={() => { addToCart(product, variant, 1, selection); track('add_to_cart', { productId: product.slug, sku: quote.sku, optionId: selection.optionId, packSize: selection.packSize, kitIncluded: quote.kitIncluded }) }} className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#071724] px-5 text-sm font-semibold text-white transition hover:bg-teal-700 focus:outline-none focus:ring-4 focus:ring-teal-200"><ShoppingCart size={16} />Add configured order to cart</button> : <a href={`/intake?product=${encodeURIComponent(product.slug)}`} className="mt-4 inline-flex min-h-12 w-full items-center justify-center rounded-full border border-slate-200 text-sm font-semibold text-[#071724]">Request availability</a>}
      <p className="mt-3 flex items-start gap-2 text-xs leading-5 text-slate-500"><Check size={14} className="mt-0.5 shrink-0 text-teal-700" />Research use only. Payment is not collected on this site; final availability and order details are reviewed through the existing process.</p>
      </>
      )}
    </section>
  )

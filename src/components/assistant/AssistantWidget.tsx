...
import { buildOrderInquiryMessage, buildWhatsAppUrl, getSupportInquiryMessage } from '../../lib/whatsapp'
...
function AssistantWidgetInner() {
  ...
  const productName = useMemo(() => getCurrentProductName(window.location.pathname), [])
  const whatsappHref = useMemo(
    () => buildWhatsAppUrl(
      productName
        ? buildOrderInquiryMessage({ product: productName, locale })
        : getSupportInquiryMessage(locale),
    ),
    [locale, productName],
  )
      ...
      {!isOpen ? (
        ...
        >
          ...
          <span className="hidden flex-col items-start leading-tight xl:flex">
            <span>{t('whatsappButtonLabel')}</span>
            <span className="mt-0.5 text-[0.65rem] font-semibold text-[#071724]/70">
              {t('whatsappReplyTime')}
            </span>
          </span>
        </a>

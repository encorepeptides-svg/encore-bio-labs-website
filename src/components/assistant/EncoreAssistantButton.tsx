...
export function EncoreAssistantButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label="Ask Encore AI"
      className="group fixed bottom-[10.5rem] right-4 z-[70] flex h-14 items-center gap-2.5 rounded-full border border-white/60 bg-white/75 py-2 pl-2.5 pr-5 text-sm font-semibold text-[var(--navy)] shadow-[0_18px_50px_rgba(26,35,64,0.18)] backdrop-blur-2xl transition duration-300 hover:-translate-y-1 hover:bg-white md:bottom-24 md:right-6"
    >
